...
# Conjunto extensible de "nombres" que NUNCA deben mandarse a LLM-DICT para
# "corrección" aunque no estén en el diccionario — porque no son nombres
# deformados, son etiquetas de rol intencionales (p. ej. "Defensor del
# Pueblo", "Ministro") usadas por módulos que reutilizan este script, como
# identificador_comisiones.py. Vacío por defecto (no afecta al pleno); un
# módulo externo puede hacer `identificador_speakers.NOMBRES_PLACEHOLDER_NO_CORREGIR.update(...)`
# tras importar este módulo.
NOMBRES_PLACEHOLDER_NO_CORREGIR: set = set()

# Partidos-etiqueta de rol externo (no parlamentario): si el partido de un
# chunk ya es uno de estos, LLM-DICT nunca debe tocarlo, aunque el nombre
# parezca deformado — buscarlo en el diccionario de diputados es la fuente
# del error, ya que esa persona no es diputada por definición.
ROLES_EXTERNOS_NO_TOCAR = {"Defensor del Pueblo", "Gobierno", "Compareciente externo"}
...
def _rellenar_partido_desde_diccionario(chunks: list) -> int:
    """Relleno gratuito (sin LLM): para cualquier chunk con nombre ya resuelto
    pero partido nulo, si ese nombre exacto está en el diccionario de
    diputados con un partido real anotado, se usa directamente. "Senador" y
    "Gobierno" no son partidos reales (son entradas sin partido anotado en
    el diccionario), así que se descartan como fuente. Debe ejecutarse ANTES
    de LLM-DESC/LLM-DICT: resuelve gratis casos como Legarda Uriarte o
    Ibáñez Hernando que ya tienen partido conocido en el diccionario pero que
    la heurística no logró extraer del texto de Mesa."""
    n = 0
    for c in chunks:
        if c.get("nombre") and c.get("partido") is None:
            partido_dict = NOMBRE_A_PARTIDO.get(c["nombre"])
            if partido_dict not in (None, "Senador", "Gobierno"):
                c["partido"] = partido_dict
                c["metodo_id"] = f"{c.get('metodo_id')}+partido_diccionario"
                n += 1
    return n


def verificar_nombres_fuera_diccionario_con_llm(chunks, fingerprints, api_key, umbral=0.78):
    """LLM-DICT: corrige nombres deformados por Whisper no presentes en el
    diccionario, Y TAMBIÉN resuelve el partido cuando el nombre ya es
    correcto pero el partido sigue sin conocerse tras el relleno gratuito
    (p. ej. nombres deformados como "Velarga"/"Gobles" que ni siquiera
    entraban antes en esta función porque venían de heuristica_cierre o
    fingerprint_retroactivo, métodos que antes no se incluían aquí)."""
    bloques = {}
    for i, c in enumerate(chunks):
        if c["estado_id"] != "IDENTIFICADO" or not c.get("nombre"):
            continue
        if c["nombre"] in NOMBRES_PLACEHOLDER_NO_CORREGIR:
            continue  # etiqueta de rol intencional (p. ej. "Defensor del Pueblo"), no un nombre deformado
        # NUNCA tocar a alguien cuyo PARTIDO ya es un rol externo (Defensor del
        # Pueblo, Gobierno, Compareciente externo), aunque el nombre en sí
        # tenga pinta de deformado — no son diputados y no deben buscarse en
        # el diccionario de diputados. Caso real detectado: el Defensor del
        # Pueblo, con nombre deformado "Ángel Babilano" pero partido correcto
        # "Defensor del Pueblo", fue "corregido" por error contra un diputado
        # real del PP con nombre parecido, perdiendo la identificación
        # correcta en 103 chunks de un vídeo real.
        if c.get("partido") in ROLES_EXTERNOS_NO_TOCAR:
            continue
        if c.get("metodo_id") not in (
            "heuristica", "fingerprint", "fingerprint_fusion",
            "heuristica_cierre", "fingerprint_retroactivo",
        ):
            continue
        # Entra si el nombre está deformado/fuera de diccionario, O si el
        # partido sigue sin resolverse (el relleno gratis ya lo habría
        # completado si estuviera en el diccionario con partido real).
        if _nombre_fuera_diccionario(c["nombre"]) or c.get("partido") is None:
            sp = c["ponente"]
            if sp not in bloques:
                bloques[sp] = []
            bloques[sp].append(i)
    ...
    for ponente, indices in bloques.items():
        ...
        # NUNCA dejar que LLM-DICT "invente" un nombre cuando no hay ni un
        # candidato razonable en el diccionario — sin candidatos, cualquier
        # nombre que proponga el LLM es una alucinación sobre texto corto o
        # genérico, no una corrección real. Caso real detectado: "Guada" (0
        # candidatos) se "corrigió" a "Guillermo Montón" | PSOE, mezclando a
        # la portavoz de Sumar con la compareciente de Greenpeace. En estos
        # casos, mejor dejarlo tal cual (ya pasó por LLM-DESC antes en el
        # flujo) que inventar una identidad sin ninguna base real.
        if not candidatos:
            print(f"[LLM-DICT] → {ponente} ('{nombre_actual}'): 0 candidatos del diccionario, "
                  f"se omite para no inventar un nombre sin base")
            continue

        texto_candidatos = "\n".join(f"- {c} ({NOMBRE_A_PARTIDO.get(c, '?')})" for c in candidatos)
...
def identificar_video(
    ...
    funcion_heuristica=None,
) -> Path:
    # funcion_heuristica: por defecto usa detectar_patrones_heuristicos (pleno).
    # Permite inyectar una heurística distinta (p. ej. la de comisiones) sin
    # duplicar todo el resto de la orquestación (fingerprint, fusión, LLM).
    # Debe tener la misma firma: funcion_heuristica(chunks, idx) -> dict.
    if funcion_heuristica is None:
        funcion_heuristica = detectar_patrones_heuristicos
    ...
    logging.info(f"Iniciando identificación v2.18: {ruta_json_entrada}")
    ...
    for idx, chunk in enumerate(chunks):
        ...
        # ── 2. Heurística ───────────────────────────────────────────────
        h = funcion_heuristica(chunks, idx)
    ...
    # ── RELLENO GRATIS: partido desde diccionario para nombres ya resueltos
    # con partido nulo (p. ej. Legarda Uriarte, Ibáñez Hernando) — no cuesta
    # ninguna llamada a Groq, así que se hace ANTES de tocar el LLM.
    n_partido_dict = _rellenar_partido_desde_diccionario(chunks)
    if n_partido_dict:
        logging.info(f"RELLENO DICCIONARIO: {n_partido_dict} chunks con partido completado")
    ...
    logging.info(
        f"FIN v2.18 — identificados={identificados}, cierre={n_cierre_retroactivo}, "
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Uso: python identificador_speakers_v2_18.py <ruta_json> [--sin-llm]")
        sys.exit(1)
